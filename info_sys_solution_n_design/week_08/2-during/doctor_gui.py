...
class DoctorGUI():
    ...
    def create_gui(self, root):
        ...
        # doctor_id is disabled to prevent user from entering a value
        # doctor_id is generated by the database because AUTOINCREMENT 
        ...
        # row 4: email label and combobox (the listbox will go through)
        tk.Label(
            form_frame, 
            text= "Email", 
            font=('arial', 10), 
        ...
        # row 5: work_phone label and combobox (the listbox will go through)
        tk.Label(
            form_frame, 
            text= "Mobile", 
            font=('arial', 10), 
    ...
    def save(self):
        ...
        #     print("Calling create() as doc_id is absent")
        #     self.create(data)
        # else:
        ...
        if valid_data:
            if (len(data['doc_id'])==0):
                # If nothing has been entered in doc_id 
    ...
    def validate_fields(self, data):
        ...
        if len(data['doc_firstname'])==0:
            valid_data = False
            message_list.append("firstname is empty")
        if len(data['doc_lastname'])==0:
            valid_data = False
            message_list.append("lastname is empty")
        if len(data['doc_email'])==0:
            valid_data = False
            message_list.append("email is empty")
        if len(data['doc_mobile'])==0:
            valid_data = False
            message_list.append("mobile is empty")
        ...
        if not self.validator.is_alphabetic(data['doc_firstname']):
            valid_data = False
            message_list.append("invalid firstname")

        if not self.validator.is_alphabetic(data['doc_lastname']):
            valid_data = False
            message_list.append("invalid lastname")
    
        # Check if email follows a certain pattern 
        # i.e contains an @ followed by a dot
        if not self.validator.is_email(data['doc_email']):
            valid_data = False
        ...
        if not self.validator.is_phone_number(data['doc_mobile']):
            valid_data = False
