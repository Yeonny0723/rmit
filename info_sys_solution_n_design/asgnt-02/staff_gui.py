...
from staff_dao import StaffDAO # To communicate with staff table
...
class StaffGUI():
    """StaffGUI class to implement CRUD functionalities & methods to manage the user interface"""
    ...
    def create_gui(self, root):
        ...
        # staff_frame = tk.Frame(root).pack() 
        ...
        # row 1: staff_id label, staff_id entry and list_of_ids label
        tk.Label(
            form_frame, 
            text= "Staff Id", 
            font=('arial', 10), 
        ...
        # staff_id is disabled to prevent user from entering a value
        # staff_id is generated by the database because AUTOINCREMENT 
        ...
        # row 5: title label and entry (the listbox will go through)
        tk.Label(
            form_frame, 
            text= "Title", 
            font=('arial', 10), 
        ...
        item_cb = ttk.Combobox(form_frame, width=28,textvariable=self.title)
        item_cb['state'] = 'readonly'
        item_cb['values'] = ['Receptionist','Staff','Manager']
        # place the widget
        item_cb.grid(row=5, column=1)

        # bind the selected value changes
        def item_changed(event):
            """ handle the month changed event """
            selected_item = self.title
            showinfo(
                title='Result',
        ...
        button_frame = tk.LabelFrame(staff_frame, pady=10, text="Commands") 
        button_frame.pack()
    ...
    def save(self):
        ...
        if (len(data['staff_id'])==0):
            print("Calling create() as staffid is absent")
            self.create(data)
        else:
            print("Calling update() as staffif is present")
            self.update(data) 
        self.load()
    ...
    def get_fields(self):
        ...
        staff = {}
        # staff_id is ignored when creating a record
        staff['staff_id'] = self.staff_id.get() 
        staff['first_name'] = self.first_name.get()
        staff['lastname'] = self.lastname.get()
    ...
    def load(self):
        ...
        if "staff_ids" in result: 
            list_ids = result['staff_ids'] # will crash if there is no entry!
            ...
            print("Setting staff_ids in listbox ...")
            for x in list_ids:
    ...
    def filter_by_lastname(self):
        ...
        result = self.staff_dao.find_by_lastname(lastname)

        print("*****result", result)
        if "filtered_staffs_ids" in result: 
            list_ids = result['filtered_staffs_ids'] # will crash if there is no entry!
            # Set the returned list into the listbox
            # Before doing that, must clear any previous list in the box
            self.lb_ids.delete(0,tk.END)
            print("Setting filtered_staffs_ids in listbox ...")
            for x in list_ids:
                self.lb_ids.insert(tk.END, x)
                #print(x)
            pass
    ...
    def on_list_select(self, evt):
        ...
        index = int(w.curselection()[0]) 
         ...
         # value of the item clicked, in our case it's the staff_id  
        value = w.get(index) 
        ...
        result = self.staff_dao.find_by_id(value)

        # { "staff" : {"staff_dao": "", "first_name": "", etc}}
        print(f"result: {result}") 

        # Grab staff dict from result dict and use it to populate the fields on the form  
        staff = result['staff']
    ...
    def populate_fields(self, staff):
        ...
        self.lastname.set(staff['lastname'])
    ...
    def validate_fields(self, data):
        ...
        # Do not check staff_id as this is generated by the database
